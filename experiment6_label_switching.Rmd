...
X0 = generate_X(latent_data$U_list, type)
data = split_into_train_and_test(X0, y, prop=0.5)
res = foreach(n = c(2, 5, 10, 20, 30, 40), .combine="c") %dopar% {
  foreach(rep = 1:3) %dopar% {
...
```{r, fig.width=7}
library(ggplot2)
load("~/MLFS/temp_label_switching_random.RData")
df = do.call("rbind", lapply(res, function(x)data.frame(diagmse = x$mse, mse = x$mse, n = x$n)))
df$iter = 1:3
